...
public class Server {
    ...
    if (message.startsWith("/shuffle") || message.startsWith("/scrumble")) {
        String[] parts = message.split(" ", 2); // Split into command and message
        if (parts.length == 2) {
            String originalMessage = parts[1];
            String shuffledMessage = shuffleMessage(originalMessage);
            message = String.format("%s: %s", id, shuffledMessage);
        } else {
            message = "Wrong shuffle command format. Use /shuffle or /scrumble followed by a message.";
        }
    }
